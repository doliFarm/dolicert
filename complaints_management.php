...
llxHeader("", $langs->trans("CompliantsManagement"));

print load_fiche_titre($langs->trans("CompliantsManagement"), '', 'dolitrace.png@dolitrace');
...
// Draft MyObject
if (! empty($conf->dolitrace->enabled) && $user->rights->dolitrace->read)
{
	$langs->load("orders");
...
if (! empty($conf->dolitrace->enabled) && $user->rights->dolitrace->read)
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."dolitrace_myobject as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
